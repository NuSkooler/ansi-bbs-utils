const EventEmitter = require('events');

const Capabilities = require('./capabilities');

const {
    normalizedTermType,
} = require('./ttype_utils');

const {
    AsSequence,
} = require('./common')

const ANSI_BBS = require('./handlers/ansi_bbs');
const VTX = require('./handlers/vtx');
const ECMA48 = require('./handlers/ecma_048');
const CTerm = require('./handlers/cterm');
const OSCHyperlink = require('./handlers/osc_hyperlink');

const ColorXTerm24Bit = require('./color_xterm_24bit');
const ColorXTerm256 = require('./color_xterm_256');
const ColorTerm16 = require('./color_16')

const ControlCodes = require('./control_codes');
const ReplyParser = require('./input/reply_parser');
const { AudioSession } = require('./audio/session');
const { probeAudio } = require('./audio/probe');

const iconv = require('iconv-lite');
const assert = require('assert');

//
//  Each Terminal works on its own copy of a capability table
//  entry. Capabilities get added and removed per session (e.g.
//  after probing the remote terminal) and must never leak into
//  another session sharing the same terminal type.
//
const copyTermTypeCaps = (entry) => {
    return {
        encoding        : entry.encoding,
        capabilities    : new Set(entry.capabilities),
        capHandlers     : Object.assign({}, entry.capHandlers),
    };
};

//  Palette indexes and RGB components are bytes; anything else
//  would put a protocol-invalid value on the wire.
const clampByte = (n) => Math.min(Math.max(n, 0), 255);

//  Parser events forwarded as-is; 'data' becomes 'input' so it
//  cannot be confused with a stream's own 'data'.
const ForwardedInputEvents = [ 'apc', 'report', 'cpr', 'reply error' ];

//
//  A class representing a remote terminal we
//  can interact with via ANSI sequences.
//
//  Output goes to |socket|. Input is optional: a host that feeds
//  inbound bytes through feed() gets terminal replies as events
//  ('device attributes', 'report', 'apc', 'cpr') and everything
//  else back as 'input' for its own key handling.
//
//  Audio: probeAudio() asks the terminal what it can do and picks a
//  back-end; |audio| is the session to play through (a do-nothing
//  session until then, or for terminals without audio).
//
//  General ANSI resource:
//  - https://gist.github.com/fnky/458719343aabd01cfb17a3a4f7296797
//  - console_codes(4) @ https://man7.org/linux/man-pages/man4/console_codes.4.html
//
module.exports = class Terminal extends EventEmitter {
    constructor(socket, options = {}) {
        super();

        this.socket = socket;
        this.options = options;

        this.ttype              = 'unknown';
        this.termClient         = 'unknown';
        this.clientVersion      = null;
        this.ctermVersion       = null;
        this.height             = 0;
        this.width              = 0;
        this.defaultTermType    = 'ansi-bbs';
        this.termTypeCaps       = copyTermTypeCaps(Capabilities[this.defaultTermType]);

        this.encoding = 'cp437';
        assert(iconv.encodingExists(this.encoding));

        //  member names for these should match cap handlers
        this.ansi               = new ANSI_BBS(this);
        this.vtx                = new VTX(this);
        this.ecma               = new ECMA48(this);
        this.cterm              = new CTerm(this);
        this['osc-hyperlink']   = new OSCHyperlink(this);

        //  not cap handler, but providing methods
        this.controlCodes       = new ControlCodes(this);

        this._addStandardMethods();

        this.input = new ReplyParser(options.input);
        this._wireInput();

        this._audio = null;
    }

    //  Terminal stuff...
    getTerminalType() {
        return this.ttype;
    }

    setTerminalType(ttype) {
        ttype = ttype.trim().toLowerCase();
        if (!ttype) {
            return false;
        }

        this.ttype = ttype;
        this._updateCapabilities();
        this._resetAudio();
        return true;
    }

    getTerminalClient() {
        return this.termClient;
    }

    setTermClient(termClient) {
        this.termClient = termClient.trim().toLowerCase();
    }

    getHeight() {
        return this.height;
    }

    setHeight(height) {
        height = parseInt(height);
        if (isNaN(height)) {
            return false;
        }

        this.height = height;
        return true;
    }

    getWidth() {
        return this.width;
    }

    setWidth(width) {
        width = parseInt(width);
        if (isNaN(width)) {
            return false;
        }

        this.width = width;
        return true;
    }

    //  Output encoding...
    getEncoding() {
        return this.encoding;
    }

    setEncoding(encoding) {
        if (iconv.encodingExists(encoding)) {
            this.encoding = encoding;
            return true;
        }

        return false;
    }

    //  Socket stuff...
    setSocket(socket) {
        this.socket = socket;
    }

    cork() {
        if (this.socket) {
            this.socket.cork();
        }
        return this;    //  chaining
    }

    uncork() {
        if (this.socket) {
            this.socket.uncork();
        }
        return this;    //  chaining
    }

    write(s, cb) {
        return this.rawWrite(this.encode(s, true), cb);
    }

    rawWrite(data, cb) {
        if (this.socket && this.socket.writable) {
            this.socket.write(data, cb);
        } else if (cb) {
            //  Never strand a caller: sequential writers key their
            //  next step off |cb|, and a write that will never happen
            //  must say so rather than hang them.
            process.nextTick(cb, new Error('Socket is not writable'));
        }
        return this;    //  chaining
    }

    //  Write an escape sequence (encoded for the terminal, no line
    //  feed conversion) and resolve once the socket has taken it.
    sendSequence(seq) {
        return new Promise((resolve, reject) => {
            this.rawWrite(this.encode(seq, false), err => err ? reject(err) : resolve());
        });
    }

    encode(s, convertLineFeeds=true) {
        if ('string' !== typeof(s)) {
            return s;
        }

        if (convertLineFeeds) {
            s = s.replace(/\n/g, '\r\n');
        }

        return iconv.encode(s, this.encoding);
    }

    //
    //  Input side. Feed every inbound chunk here; terminal replies
    //  become events and the rest comes back as 'input'.
    //
    feed(chunk) {
        this.input.feed(chunk);
        return this;    //  chaining
    }

    flushInput() {
        this.input.flush();
        return this;    //  chaining
    }

    //
    //  Audio
    //
    get audio() {
        if (!this._audio) {
            const backend = this.hasCapability('cterm-audio') ? 'cterm' : 'null';
            this._audio = new AudioSession(this, Object.assign({}, this.options.audio, { backend }));
        }
        return this._audio;
    }

    //  Ask the terminal what it can play; (re)select the audio session.
    async probeAudio(options) {
        const result = await probeAudio(this, options);
        this._resetAudio();
        return result;
    }

    destroy() {
        this._resetAudio();
        this.input.destroy();
        this.removeAllListeners();
    }

    //  Capabilities
    getCapabilities() {
        return this.termTypeCaps.capabilities;
    }

    hasCapability(cap) {
        return this.getCapabilities().has(cap);
    }

    addCapabilities(...caps) {
        caps.forEach(cap => this.getCapabilities().add(cap));
        return this;    //  chaining
    }

    addCapability(cap) {
        return this.addCapabilities(cap);
    }

    removeCapability(cap) {
        return this.getCapabilities().delete(cap);
    }

    //
    //  Foreground/Background color methods:
    //
    //  Accept args in two major forms, with or without AsSequence:
    //  1 - (r, g, b)   = 24bit color / true color
    //  2 - (n)         = '8bit-color' (a subset being standard 16 colors)
    //  3 - (name)      = 'red', 'magentaBG', ... (see ansi_bbs.js)
    //
    //  Next, try to do the best that we can:
    //  - Try to choose the right method from caps
    //  - Fall back/map from 24-bit/true > 8bit/256 -> 16c
    //  - If 8-bit and the range allows it, just use sgr(n)
    //
    fgColor(...args) {
        return this._color(args, true);
    }

    bgColor(...args) {
        return this._color(args, false);
    }

    //  Aliases to fg/bgColor() via RGB
    fgRGB(r, g, b, asSequence = false) {
        return this.fgColor(r, g, b, asSequence);
    }

    bgRGB(r, g, b, asSequence = false) {
        return this.bgColor(r, g, b, asSequence);
    }

    hyperlink(url, text = '', asSequence = false) {
        const capHandler = this._getCapHandler('hyperlink');
        const seq = capHandler ? capHandler.hyperlink(url, text, AsSequence) : (text || url);

        if (AsSequence === asSequence) {
            return seq;
        }

        this.write(seq);
        return this;
    }

    //  Private methods...
    _wireInput() {
        this.input.on('data', data => this.emit('input', data));

        ForwardedInputEvents.forEach(event => {
            this.input.on(event, payload => this.emit(event, payload));
        });

        this.input.on('device attributes', da => {
            if (da.client) {
                this.setTermClient(da.client);
            }
            if (da.version) {
                this.clientVersion = da.version;
            }
            if (da.ctermVersion) {
                this.ctermVersion = da.ctermVersion;
            }
            this.emit('device attributes', da);
        });
    }

    _resetAudio() {
        if (this._audio) {
            this._audio.destroy();
            this._audio = null;
        }
    }

    _color(args, foreground) {
        //
        //  |args| is one of:
        //  - (n | name)
        //  - (r, g, b)
        //  ...optionally followed by AsSequence, or by a false/undefined
        //  placeholder from the fg/bgRGB() default parameters.
        //
        args = args.slice();

        let asSequence = false;
        const last = args[args.length - 1];
        if (AsSequence === last) {
            asSequence = AsSequence;
            args.pop();
        } else if ((2 === args.length || 4 === args.length) && (false === last || undefined === last)) {
            args.pop();
        }

        let seq = '';
        if (1 === args.length) {
            //  3/4-bit, 8-bit/256, or a name
            seq = this._colorIndexed(args[0], foreground);
        } else if (3 === args.length) {
            //  RGB/24-bit/true color
            seq = this._color24Bit(args[0], args[1], args[2], foreground);
        }

        if (AsSequence === asSequence) {
            return seq;
        }

        if (seq) {
            this.rawWrite(seq);
        }
        return this;
    }

    _colorIndexed(n, foreground) {
        //  Allow |n| to be a string such as 'red'
        //  or 'magentaBG' (see ansi_bbs.js); a numeric
        //  string is just a number.
        if ('string' === typeof(n) && /^\s*\d+\s*$/.test(n)) {
            n = parseInt(n);
        }

        if ('string' === typeof(n)) {
            let name = n;
            //  Names are foreground by nature; a background request
            //  for 'red' means 'redBG' when such an attribute exists.
            if (!foreground && !name.endsWith('BG') && ANSI_BBS.hasSGRAttribute(`${name}BG`)) {
                name = `${name}BG`;
            }
            if (!ANSI_BBS.hasSGRAttribute(name)) {
                return '';
            }
            return this.ansi.sgr(name, AsSequence);
        }

        n = parseInt(n);
        if (isNaN(n)) {
            return '';
        }
        n = clampByte(n);

        //  Fit in the most basic 3/4-bit indexed palette
        //  and escape sequence if we can.
        if (n >= 0 && n <= 7) {
            //  e.g. 5 -> ESC[35m / ESC[45m
            const base = foreground ? 30 : 40;
            return this.ansi.sgr(base + n, AsSequence);
        }

        //  Can we even do 8-bit/256?
        if (this.hasCapability('8bit-color')) {
            return foreground ? ColorXTerm256.fgColor(n) : ColorXTerm256.bgColor(n);
        }

        //  Basic 16c
        return foreground ?
            ColorTerm16.fgColor(n) :
            ColorTerm16.bgColor(n);
    }

    _color24Bit(r, g, b, foreground) {
        r = parseInt(r);
        g = parseInt(g);
        b = parseInt(b);
        if (isNaN(r) || isNaN(g) || isNaN(b)) {
            return '';
        }
        r = clampByte(r);
        g = clampByte(g);
        b = clampByte(b);

        //  True color if we can
        if (this.hasCapability('24bit-color')) {
            return foreground ?
                ColorXTerm24Bit.fgRGB(r, g, b) :
                ColorXTerm24Bit.bgRGB(r, g, b);
        }

        //  Else if we can do 8-bit, do that.
        if (this.hasCapability('8bit-color')) {
            return foreground ?
                ColorXTerm256.fgRGB(r, g, b) :
                ColorXTerm256.bgRGB(r, g, b);
        }

        //  Finally fall back to very basic 16c
        return foreground ?
            ColorTerm16.fgRGB(r, g, b) :
            ColorTerm16.bgRGB(r, g, b);
    }

    _getCapHandler(cap) {
        let handler = this.termTypeCaps.capHandlers[cap];
        if (!handler) {
            return;
        }

        return this[handler];
    }

    _addStandardMethods() {
        //  ANSI-BBS methods are brought over directly so we have
        //  some nice shortcuts like this.cuu(), this.goto(), etc.
        //  that directly perform output

        [ this.ansi, this.controlCodes ].forEach(inst => {
            const methods = Object.getOwnPropertyNames(Object.getPrototypeOf(inst))
                .filter(name => {
                    return 'constructor' !== name && !name.startsWith('_');
                })
                .concat(Object.getOwnPropertyNames(inst))
                .filter(name => 'function' === typeof(inst[name]));

            methods.forEach(method => {
                //  never shadow our own (or EventEmitter's) members
                assert(!(method in this), `handler method "${method}" would shadow Terminal.${method}`);

                this[method] = (...args) => {
                    return inst[method](...args);
                }
            });
        });
    }

    _updateCapabilities() {
        //  :TODO: specific client -> ttype -> first
        const termType = normalizedTermType(this.ttype);

        const entry =
            Capabilities[termType] ||
            Capabilities[this.defaultTermType] ||
            Capabilities['ansi-bbs'];

        this.termTypeCaps = copyTermTypeCaps(entry);

        this.setEncoding(this.termTypeCaps.encoding);
    }
};
