...
module.exports = class TerminalOutput {
    ...
    //  :TODO: allow color(..., AsString) -> str vs output() ... can use a Symbol for this?
    fgColor(...args) {
        //
        ...
        //  - If 8-bit and the range allow it, just use sgr(n)
        //
        switch (args.length) {
            case 1 :
                //  8-bit/256
    ...
    _color8Bit(n, foreground) {
        ...
        if (this.termTypeCaps.capabilities.includes('8bit-color')) {
            return `${CSI}${foreground ? 38 : 48};5;${n}m`;
        }
    ...
    _color24Bit(r, g, b, foreground) {
        ...
        if (this.termTypeCaps.capabilities.includes('24bit-color')) {
            return `${CSI}${foreground ? 38 : 48};2;${r};${g};${b}m`;
        }

        //  :TODO: if 8-bit, map to that else 16
    }
